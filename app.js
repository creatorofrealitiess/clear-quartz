// ═══════════════════════════════════════════════════════
// CLEAR QUARTZ APP — Firebase Integration & Core Logic
// ═══════════════════════════════════════════════════════

const firebaseConfig = {
    apiKey: "AIzaSyB6XUYwJXYGEXz9xLZRGYC4JwlH1DPIrfA",
    authDomain: "clear-quartz-app.firebaseapp.com",
    projectId: "clear-quartz-app",
    storageBucket: "clear-quartz-app.firebasestorage.app",
    messagingSenderId: "1096413510793",
    appId: "1:1096413510793:web:cc9a62973fc8d3d43bda58"
};
...
document.getElementById('authToggleLink').addEventListener('click', () => {
    ...
    document.getElementById('authSubmit').textContent = isSignUp ? 'Create Account' : 'Enter Clear Quartz';
...
document.getElementById('authSubmit').addEventListener('click', async () => {
    ...
    try {
        if (isSignUp) {
            ...
            if (!displayName) { errorEl.textContent = 'Please enter your Clear Quartz name'; btn.disabled = false; btn.textContent = 'Create Account'; return; }
        ...
        btn.disabled = false;
        btn.textContent = isSignUp ? 'Create Account' : 'Enter Clear Quartz';
    }
});
...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        const savedKey = localStorage.getItem('clearquartz_gemini_key');
...
function signOut() { if (confirm('Sign out of Clear Quartz?')) auth.signOut(); }

function saveGeminiKey() {
    const key = document.getElementById('geminiApiKey').value.trim();
    if (key) { localStorage.setItem('clearquartz_gemini_key', key); alert('Gemini API key saved!'); }
}
...
const subtitles = {
    home: 'clarity lives here', world: "the world you've built", personas: 'who we are in clear quartz',
    connections: 'your people', space: 'memories & moments', journal: 'write from within',
    messenger: 'conversations in clear quartz', gallery: 'memories & moments', timeline: 'the story so far',
...
const defaultAffirmations = [
    "I don't hope for Clear Quartz. I don't wish for it. I live it \u2014 it is simply where I am.",
    ...
    "Every detail I add makes Clear Quartz more vivid, more solid, more mine.",
...
function renderAlignmentChart() {
    ...
    // Assign colors: current user gets quartz, other gets rose
...
function renderJournalList() {
    ...
    if (journalEntries.length === 0) {
        container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">\uD83D\uDCD6</div><p>No entries yet. Tap + to write your first moment from within Clear Quartz.</p></div>';
        return;
    }
...
function initMessenger() {
    ...
    loadUserData().then(() => {
        ...
        if (snapshot.empty) {
            container.innerHTML = '<div class="empty-state" style="padding-top:60px"><div class="empty-state-icon">\uD83D\uDCAC</div><p>Start talking as your Clear Quartz selves. This is your space together.</p></div>';
            return;
        }
...
function renderPersonasList() {
    ...
    if (personasData.length === 0) {
        container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">&#129695;</div><p>No personas yet. Tap + to define your Clear Quartz self.</p></div>';
        return;
    }
...
function renderConnectionsList() {
    ...
    if (connectionsData.length === 0) {
        container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">&#128154;</div><p>No connections yet. Tap + to add the first soul in your Clear Quartz world.</p></div>';
        return;
    }
...
function renderPalettes() {
    ...
    if (anchorsData.palettes.length === 0) {
        container.innerHTML = '<div class="empty-state" style="padding:var(--space-xl) 0"><div class="empty-state-icon">&#127912;</div><p>No palettes yet. Pick colours that feel like Clear Quartz.</p></div>';
        return;
    }
...
function renderScents() {
    ...
    if (anchorsData.scents.length === 0) {
        container.innerHTML = '<div class="empty-state" style="padding:var(--space-xl) 0"><div class="empty-state-icon">&#10024;</div><p>No senses yet. Describe what you experience in Clear Quartz.</p></div>';
        return;
    }
...
async function spaceGenerate() {
    ...
    const apiKey = localStorage.getItem('clearquartz_gemini_key');
...
function renderSceneList() {
    ...
    if (scenesData.length === 0) {
        container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">&#127912;</div><p>No scenes yet. Tap + to capture your first moment from Clear Quartz.</p></div>';
        return;
    }
...
function renderTimelineList() {
    ...
    if (timelineData.length === 0) {
        container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">&#10024;</div><p>No moments yet. Tap + to begin the story of your life in Clear Quartz.</p></div>';
        return;
    }
...
const VAPID_KEY = 'BErKQMtgnFRYP63tFzMVW3Gy64J-CasjRT1VtCaYnfR_91ei7bJyylFeyilBj3bZ34jkni3KlcxG87OmHGVjjHg';
...
async function saveToken(reg) {
    try {
        ...
        if (token) {
            ...
            localStorage.setItem('clearquartz_fcm_token', token);
        }
    } catch (err) {
...
async function updateNotifPreference(key, value) {
    const token = localStorage.getItem('clearquartz_fcm_token');
    if (!token) return;
